"""Template + utilitaires partagés pour la génération des articles 'Afrique hubs'."""
import json
import re
...
h1.title{font-size:1.5rem;line-height:2rem;font-weight:900;color:#0B1F3A;margin:0 0 .5rem;padding-bottom:.75rem;border-bottom:2px solid #F59E0B}
.byline{font-size:.8125rem;color:#6B7280;margin:0 0 1.5rem;font-weight:600}
.byline strong{color:#0B1F3A;font-weight:700}
.tldr{margin:0 0 1.75rem;padding:1rem 1.25rem;border-radius:.625rem;background:linear-gradient(135deg,#FFF9E6 0%,#FFF7ED 100%);border-left:4px solid #F59E0B}
...
.cta-box a{color:#F59E0B;font-weight:700;margin:0 .5rem;text-decoration:none}
...
.related a{color:#0B6BA3;font-weight:600;text-decoration:none}
.related a:hover{color:#F59E0B;text-decoration:underline}
...
#blog-body h2{font-size:1.125rem;font-weight:700;color:#0B1F3A;margin:1.75rem 0 .5rem;padding-left:12px;border-left:4px solid #F59E0B}
...
#blog-body li::marker{color:#F59E0B}
#blog-body strong{color:#0B1F3A}
#blog-body a{color:#0B6BA3;font-weight:600}
#blog-body a:hover{color:#F59E0B;text-decoration:underline}
...
#blog-body blockquote{border-left:4px solid #F59E0B;background:#FFF9E6;padding:12px 16px;border-radius:0 8px 8px 0;margin:1rem 0;color:#0B1F3A}
.faq{margin-top:2rem;border:1px solid #E5E7EB;background:#fff;border-radius:.75rem;padding:1.25rem 1.5rem}
.faq h2{font-size:1.125rem;font-weight:700;color:#0B1F3A;margin:0 0 .75rem;padding-left:12px;border-left:4px solid #F59E0B}
...
.faq summary::after{content:'+';position:absolute;right:0;top:50%;transform:translateY(-50%);font-size:1.25rem;color:#F59E0B;font-weight:700;line-height:1}
.faq details[open] summary::after{content:'−'}
.faq details>div{margin-top:.5rem;font-size:.875rem;color:#374151;line-height:1.6}
.faq details>div a{color:#0B6BA3;font-weight:600}
...
def build_blogposting(art):
    return {
        "@context": "https://schema.org",
        "@type": "BlogPosting",
        "headline": art["title"],
        "description": art["description"],
        "url": f"https://robindesairs.eu/blog/{art['slug']}.html",
        "image": "https://robindesairs.eu/og-blog.png",
        "datePublished": art.get("datePublished", "2026-05-26"),
        "dateModified": art.get("dateModified", "2026-05-26"),
        "mainEntityOfPage": {"@type": "WebPage", "@id": f"https://robindesairs.eu/blog/{art['slug']}.html"},
        "author": {"@type": "Organization", "@id": "https://robindesairs.eu/#organization", "name": "Robin des Airs", "url": "https://robindesairs.eu/"},
        "publisher": {
            "@type": "Organization", "@id": "https://robindesairs.eu/#organization", "name": "Robin des Airs",
            "alternateName": "Robin des Airs Compensation Aérienne", "url": "https://robindesairs.eu/",
            "logo": {"@type": "ImageObject", "url": "https://robindesairs.eu/robin-des-airs-logo-texte-profil.png"},
            "disambiguatingDescription": "Service de récupération d'indemnités aériennes (règlement CE 261/2004, Convention de Montréal), spécialiste de l'axe Europe-Afrique. À ne pas confondre avec des entités homonymes opérant dans d'autres secteurs.",
        },
        "inLanguage": "fr-FR",
        "isPartOf": {"@type": "Blog", "name": "Blog Robin des Airs", "url": "https://robindesairs.eu/blog/"},
    }


def build_breadcrumb(art):
    return {
        "@context": "https://schema.org",
        "@type": "BreadcrumbList",
        "itemListElement": [
            ...
            {"@type": "ListItem", "position": 3, "name": art["title"], "item": f"https://robindesairs.eu/blog/{art['slug']}.html"},
        ],
    }
...
def build_faq(art):
    return {
        "@context": "https://schema.org",
        "@type": "FAQPage",
        "mainEntity": [{"@type": "Question", "name": q["q"], "acceptedAnswer": {"@type": "Answer", "text": strip_html(q["a"])}} for q in art["faq"]],
    }
...
def render(art):
    ...
    slug = art["slug"]
    url = f"https://robindesairs.eu/blog/{slug}.html"
    h1 = art.get("h1", title)
    qa = art["quick_answer"]
    body = art["body_html"]
    faq_items = art["faq"]
    related = art["related"]
    pub = art.get("datePublished", "2026-05-26")
    mod = art.get("dateModified", "2026-05-26")

    bp = json.dumps(build_blogposting(art), ensure_ascii=False)
    bc = json.dumps(build_breadcrumb(art), ensure_ascii=False)
    fq = json.dumps(build_faq(art), ensure_ascii=False)

    tldr = "\n        ".join(f"<li><strong>{q['q']}</strong> — {truncate(q['a'])}</li>" for q in faq_items)
    faq_html = "\n      ".join(f"<details>\n        <summary>{q['q']}</summary>\n        <div>{q['a']}</div>\n      </details>" for q in faq_items)
    rel_html = "\n        ".join(f'<li><a href="{r["href"]}">{r["label"]}</a></li>' for r in related)
...
<head>
  ...
  <meta property="og:image" content="https://robindesairs.eu/og-blog.png">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="{title}">
  <meta name="twitter:description" content="{desc}">
  <meta name="twitter:image" content="https://robindesairs.eu/og-blog.png">
...
<body>
  <nav>
    <a href="/">ROBIN<span style="color:#F59E0B"> des Airs</span></a>
    <div><a href="/" class="back">← Retour</a></div>
  </nav>
  <main class="wrap">
    <h1 class="title">{h1}</h1>
    <p class="byline">Par <strong>l'équipe Robin des Airs</strong> · Publié le <time datetime="{pub}">26 mai 2026</time> · Mis à jour le <time datetime="{mod}">26 mai 2026</time></p>
    <aside class="quick-answer" role="complementary" aria-label="Réponse rapide">
      <span class="qa-label">Réponse rapide</span>
      <p>{qa}</p>
    </aside>
    <aside class="tldr" role="complementary" aria-label="En bref">
      <h2>En bref</h2>
      <ul>
        {tldr}
      </ul>
    ...
    <p class="signature">Article rédigé et vérifié par <strong>l'équipe Robin des Airs</strong> (robindesairs.eu) — spécialistes des indemnités aériennes CE 261 sur l'axe Europe-Afrique. <span class="disambig">À ne pas confondre avec d'autres entités utilisant un nom similaire dans le secteur environnemental.</span></p>
    <p class="disclaimer"><strong>Information générale.</strong> Cet article présente une synthèse pédagogique de la réglementation en vigueur (<a href="https://eur-lex.europa.eu/legal-content/FR/TXT/?uri=CELEX%3A32004R0261" target="_blank" rel="noopener external">règlement CE 261/2004</a>, <a href="https://www.icao.int/secretariat/legal/list%20of%20parties/mtl99_fr.pdf" target="_blank" rel="noopener external">Convention de Montréal</a>, jurisprudence CJUE) à la date de publication. Il ne constitue pas un conseil juridique personnalisé ni une consultation d'avocat. Pour l'évaluation de votre situation individuelle, contactez Robin des Airs (mandat de représentation) ou un avocat spécialisé en droit aérien. Les montants, délais et exemples cités sont indicatifs et peuvent évoluer selon les décisions de justice et l'actualité réglementaire.</p>
  </main>
</body>
</html>
"""
...
def write_all(articles, lot_name):
    written = []
    for art in articles:
        path = BLOG / f"{art['slug']}.html"
        path.write_text(render(art), encoding="utf-8")
        written.append(art["slug"])
    print(f"[{lot_name}] {len(written)} articles ecrits:")
    for s in written:
        print(f"  - {s}.html")
    return written
