...
# Page mandat statique (même document que public/mandat.html sur le site)
...
LIENS :
- Mandat: robindesairs.eu/mandat.html (prérempli depuis WhatsApp)
...
def generate_ref_dossier(phone):
    """Référence dossier alignée avec le mandat / Airtable : RDA-YYYYMMDD-XXXX."""
...
def _mandat_wa_display(phone):
    """Numéro WhatsApp lisible pour préremplir le mandat (champ wa)."""
...
def _mandat_fn_ln_from_names(names):
    """Déduit prénom / nom du 1er passager 'Prénom NOM' pour préremplir le mandat."""
    if not names:
        return "", ""
...
def show_summary_and_mandat(phone, conv):
    ...
    # Lien mandat = même page que mandat.html (query lues par le script du mandat)
...
def test():
    return jsonify({
        "status": "running",
        "version": "v5 - lien mandat.html + ref dossier dans l'URL",
        "features": [
