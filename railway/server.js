...
- ⚠️ ESCALE / VIA : si le document indique « VIA <ville/aéroport> », « CORRESPONDANCE », « CONNECTING », « CNX », ou fait apparaître 3 aéroports (ou 2 numéros de vol), le voyage comporte une ESCALE = AU MOINS DEUX tronçons. Mets alors depart = 1er aéroport et arrivee = DERNIER aéroport (destination FINALE, jamais l'escale), et dans "segments" décompose : 1er tronçon départ→escale, 2e tronçon escale→arrivée finale. Si le n° d'un tronçon n'est pas imprimé, mets son "vol":"" mais renseigne quand même son depart/arrivee. N'OUBLIE JAMAIS la ville « via » : c'est ce 2e tronçon qui est le plus souvent manqué, alors qu'il désigne le vol vers l'Europe (celui qui ouvre les droits).
...
const _OCR_PASSPORT_PROMPT = `Tu lis une pièce d'identité (PASSEPORT, carte nationale d'identité, titre de séjour, carte de résident…).
...
function _normalizePassportOcr(p) {
  ...
  // VIDE (à confirmer) plutôt qu'une date fausse.
...
async function handleMessage(phone, text, cfg, mediaUrl, replyId, _retried, referral) {
  ...
  if (s.step === 'incident') {
    ...
    if (id === 'inc_retard' || n === '1' || lower.includes('retard') || lower.includes('delay')) { s.step = 'duree'; await setState(phone, s); return sendButtons(phone, { body: L(s, `😟 Sorry about this delay. Did your flight have a delay of *3 hours or more*?`, `😟 Désolé pour ce retard. Votre vol a-t-il eu *3 heures ou plus* de retard ?`), buttons: [{ id: 'dur_plus', text: L(s, '✅ 3h or more', '✅ 3 h ou plus') }, { id: 'dur_moins', text: L(s, '❌ Less than 3h', '❌ Moins de 3 h') }, { id: 'dur_inconnu', text: L(s, '🤔 Not sure', '🤔 Je ne sais pas') }] }, cfg); }
  ...
  if (s.step === 'duree') {
    ...
    if (await stuckHelp(phone, s, cfg)) return;
    return sendButtons(phone, { body: L(s, `🙂 Tap a button: did your flight have a delay of 3 hours or more?`, `🙂 Touchez un bouton : votre vol a-t-il eu 3 heures ou plus de retard ?`), buttons: [{ id: 'dur_plus', text: L(s, '✅ 3h or more', '✅ 3 h ou plus') }, { id: 'dur_moins', text: L(s, '❌ Less than 3h', '❌ Moins de 3 h') }, { id: 'dur_inconnu', text: L(s, '🤔 Not sure', '🤔 Je ne sais pas') }] }, cfg);
  }
  ...
  if (s.step === 'type_vol') {
    ...
    return sendButtons(phone, { body: L(s, `${bar('scan')}\n📸 Send a *photo* of your ticket — I'll handle everything.\n\n_ℹ️ Automatic AI reading: your details pre-fill your claim contract by themselves, you type nothing._\n\n📎 Send the photo, or:`, `${bar('scan')}\n📸 Envoyez une *photo* de votre billet — je m'occupe de tout.\n\n_ℹ️ Lecture automatique par IA : vos informations pré-remplissent toutes seules votre contrat de réclamation, vous ne tapez rien._\n\n📎 Envoyez la photo, ou :`), buttons: [{ id: 'scan_photo', text: L(s, '📸 Send a photo', '📸 Envoyer une photo') }, { id: 'scan_manuel', text: L(s, '✏️ Type it in', '✏️ Saisir à la main') }] }, cfg);
  }
...
async function sendAccueil(phone, cfg, lang, referral) {
  ...
  await sendButtons(phone, { body: en
    ...
    footer: 'CE 261/2004', buttons: [{ text: '🇫🇷 Français' }, { text: '🇬🇧 English' }] }, cfg);
